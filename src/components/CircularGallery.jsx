...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          // contain image (like object-fit: contain)
          vec2 imageAspect = uImageSizes / max(uImageSizes.x, uImageSizes.y);
          vec2 planeAspect = uPlaneSizes / max(uPlaneSizes.x, uPlaneSizes.y);
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.plane.scale.x = this.plane.scale.y * 0.75; // keep card proportion
    this.plane.program.uniforms.uPlaneSizes.value = [this.plane.scale.x, this.plane.scale.y];
    this.padding = 2;
    this.width = this.plane.scale.x + this.padding;
    this.widthTotal = this.width * this.length;
...
class App {
  ...
  createMedias(items, bend = 1, textColor, borderRadius, font) {
    const defaultItems = [
      ...
      { image: `/images/threejs.png`, text: 'Three JS' },
