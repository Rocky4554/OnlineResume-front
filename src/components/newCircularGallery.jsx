...
//     // Position text below icon plane
...
//     this.scroll.target +=
//       (delta > 0 ? this.scrollSpeed : -this.scrollSpeed) * 0.3;
...
//         skill.onResize({ screen: this.screen, viewport: this.viewport })
//       );
//     }
//   }
...
class Title {
  constructor({ gl, parent, text, textColor, font }) {
    ...
    this.font = font || "bold 28px Arial";
  ...
  createTextMesh() {
    this.mesh = new Mesh(this.gl, {
      geometry: new Plane(this.gl, { width: 1.8, height: 0.5 }),
      program: new Program(this.gl, {
        vertex: `
          precision highp float;
          attribute vec3 position;
...
// ===================== SkillMedia Class =====================
class SkillMedia {
  constructor({ gl, texture, text, textColor, font }) {
    this.gl = gl;
...
export default function CircularGallery() {
  ...
  useEffect(() => {
    ...
    Promise.all(
      skills.map(skill =>
        new Promise(resolve => {
          ...
          img.onload = () => {
            ...
            const skillMedia = new SkillMedia({
              gl,
              texture,
              text: skill.name,
              textColor: "#fff",
            ...
            skillMedia.plane.setParent(scene);
            skillPlanes.push(skillMedia.plane);
            resolve();
          };
        })
      )
      ...
      const radius = 6;
      skillPlanes.forEach((plane, i) => {
        const angle = (i / skillPlanes.length) * Math.PI * 2;
